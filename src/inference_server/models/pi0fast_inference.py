...
class Pi0FastInferenceEngine(BaseInferenceEngine):
    ...
    async def load_policy(self):
        ...
        try:
            ...
            logger.exception(f"Failed to load Pi0Fast policy from {self.policy_path}")
            msg = f"Failed to load Pi0Fast policy: {e}"
    ...
    async def predict(
        ...
        try:
            ...
            with torch.no_grad():
                ...
                logger.debug(f"Pi0Fast predicted action shape: {action.shape}")
                return action

        except Exception as e:
            logger.exception("Pi0Fast inference failed")
            msg = f"Pi0Fast inference failed: {e}"
            raise RuntimeError(msg) from e
    ...
    def get_model_info(self) -> dict:
        """Get Pi0Fast-specific model information."""
