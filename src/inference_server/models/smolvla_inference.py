...
class SmolVLAInferenceEngine(BaseInferenceEngine):
    ...
    async def load_policy(self):
        ...
        try:
            ...
            logger.exception(f"Failed to load SmolVLA policy from {self.policy_path}")
            msg = f"Failed to load SmolVLA policy: {e}"
    ...
    async def predict(
        ...
        try:
            ...
            with torch.no_grad():
                ...
                logger.debug(f"SmolVLA predicted action shape: {action.shape}")
                return action

        except Exception as e:
            logger.exception("SmolVLA inference failed")
            msg = f"SmolVLA inference failed: {e}"
            raise RuntimeError(msg) from e
    ...
    def get_model_info(self) -> dict:
        """Get SmolVLA-specific model information."""
